<?php
	...
	if(isset($_POST['register'])){
		...
		$username_sql = "SELECT * FROM nurse WHERE nUsername = '$username'";
		$check_user = mysqli_query($conn,$username_sql);
				
		$email_sql = "SELECT * FROM nurse WHERE nEmail = '$email'";
		...
		if (mysqli_num_rows($check_user) > 0){
			...
			$_SESSION['status']="Email is already used.";
				...
				if($password == $cpass){
				$reg_sql = "INSERT INTO nurse (nFullName, nUsername, nEmail,
				nNumber, nPassword) VALUES ('".$fullname."','".$username."','".$email."','".$cnum."','".$password."')";
