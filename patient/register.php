<?php
	...
	if(isset($_POST['register'])){
		...
		$username_sql = "SELECT * FROM patient WHERE pUsername = '$username'";
		$check_user = mysqli_query($conn,$username_sql);
				
		$email_sql = "SELECT * FROM patient WHERE pEmail = '$email'";
		...
		if (mysqli_num_rows($check_user) > 0){
			...
			$_SESSION['status']="Email is already used.";
				...
				if($password == $cpass){
				$reg_sql = "INSERT INTO patient (pFullName, pUsername, pEmail,
				pNumber, pPassword) VALUES ('".$fullname."','".$username."','".$email."','".$cnum."','".$password."')";
